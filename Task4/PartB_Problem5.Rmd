...
map_data_part <- resample_partition(mapdata1,
                                    p=c(train=0.5,
                                        valid=0.25,
                                        test=0.25))
fit_t1 <- lm(log(life_expectancy_years) ~ log(medical_doctors_per_1000_people), data=map_data_part$train)

fit_t2 <- lm(log(life_expectancy_years) ~ log(medical_doctors_per_1000_people + poverty_percent_people_below_550_a_day), data=map_data_part$train)

fit_t3 <- lm(log(life_expectancy_years) ~ log(medical_doctors_per_1000_people + poverty_percent_people_below_550_a_day + murder_per_100000_people), data=map_data_part$train)

rmse(fit_t1, map_data_part$valid)
rmse(fit_t2, map_data_part$valid)
rmse(fit_t3, map_data_part$valid)

rmse(fit_t3, map_data_part$test)
```